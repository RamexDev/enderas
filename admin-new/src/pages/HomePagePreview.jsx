  ...
  return (
    <div className="bg-sand-50">
        ...
        {slides.length === 0 ? (
          ...
          slides.map((slide, idx) => (
            <EditOverlay
              ...
              record={{ id: slide.id }}
              pageKey={pageKey}
              className={idx === 0 ? 'absolute inset-0' : 'hidden'}
            >
              <HeroSlide slide={slide} active={idx === 0} />
            </EditOverlay>
          ))
        )}
      </section>
              ...
              <div className="mt-8 grid gap-5 sm:grid-cols-2">
                {INTRO_PILLARS.map((f) => (
                  <div key={f.title} className="flex items-start gap-3">
                    ...
                    <div>
                      <h3 className="text-sm font-semibold text-primary-900">{f.title}</h3>
                      <p className="mt-0.5 text-xs leading-relaxed text-primary-700/90">{f.body}</p>
                    </div>
                  </div>
                ))}
              </div>
              {intro.cta?.label && (
                <span className="mt-8 inline-flex items-center gap-2 rounded-lg border border-primary-300 px-5 py-2.5 text-sm font-medium text-primary-900">
                  {intro.cta.label}
                </span>
              )}
            </EditOverlay>
            ...
            <div className="lg:col-span-6">
              <div className="grid grid-cols-2 gap-3 sm:gap-4">
                <div className="space-y-3 sm:space-y-4">
                  <div className="aspect-[4/5] overflow-hidden rounded-2xl bg-primary-100">
                    <img src={INTRO_IMAGES[0]} alt="" className="h-full w-full object-cover" loading="lazy" />
                  </div>
                  <div className="aspect-square overflow-hidden rounded-2xl bg-primary-100">
      ...
      {visibility.showTestimonials && (
        <section className="section-padding bg-sand-100/60">
          <Container>
      ...
      {/* If testimonials/faq are hidden, surface the visibility editor at the bottom */}
      ...
      <Container className="py-6">
        <EditOverlay section={sections.seoSection} pageKey={pageKey}>
          <div className="flex items-center gap-2 rounded-lg border border-primary-100 bg-white px-4 py-2.5 text-xs text-primary-500">
            <span className="font-medium text-primary-700">SEO:</span>
...
function EmptyHero() {
  return (
    <Container className="relative flex h-full flex-col items-center justify-center text-center text-primary-200">
      <p className="text-sm">No hero slides yet.</p>
      <p className="mt-1 text-xs text-primary-300">Click the 'Hero slides' button to add the first one.</p>
    </Container>
  )
}
