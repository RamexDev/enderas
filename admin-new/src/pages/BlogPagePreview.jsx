    ...
    <div className="bg-sand-50 dark:bg-primary-950">
      ...
      <section className="section-padding">
        <Container>
          ...
          <div className="mb-8 flex flex-wrap items-center justify-between gap-4">
            <div>
              <h2 className="font-heading text-xl font-semibold text-primary-900 dark:text-white">All posts</h2>
          ...
          {drafts.length > 0 && (
            <div className="mb-12">
              <div className="mb-4 flex items-center gap-2">
                ...
                <span className="text-xs text-primary-500 dark:text-primary-400">Not visible to the public</span>
              </div>
          ...
          {published.length === 0 && drafts.length === 0 ? (
            ...
            <div>
              <div className="mb-4 flex items-center gap-2">
                ...
                <span className="text-xs text-primary-500 dark:text-primary-400">Visible on /blog</span>
              </div>
              <div className="grid gap-5 sm:gap-6 md:grid-cols-2 lg:grid-cols-3">
                {published.map((p) => (
                  <EditOverlay
      ...
      {published.length > 0 && (
        <Container className="pb-12">
          <div className="rounded-xl border border-primary-100 bg-white p-5 dark:border-primary-800 dark:bg-primary-900">
            <h3 className="mb-3 text-sm font-semibold text-primary-900 dark:text-white">Most recent posts</h3>
            <ul className="space-y-2 text-xs">
              {published.slice(0, 5).map((p) => (
                <li key={p.id} className="flex items-center justify-between gap-3 text-primary-600 dark:text-primary-300">
                  <span className="truncate">{p.title}</span>
                  <span className="shrink-0 text-primary-400 dark:text-primary-500">{formatDate(p.date)}</span>
                </li>
              ))}
            </ul>
          </div>
